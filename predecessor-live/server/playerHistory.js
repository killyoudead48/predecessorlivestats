...
export async function getPlayerMatchHistory(playerId) {
  try {
    const matchesRes = await fetch(`https://api.prod.omeda.city/v1/matches/get-matches-by-player?player_id=${playerId}&limit=10`);
    ...
    const heroesRes = await fetch('https://api.prod.omeda.city/v1/heroes/list');
    if (!heroesRes.ok) {
